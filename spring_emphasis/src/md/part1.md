...
循环依赖就跟递归一样，如果没有出口就会不停地循环下去，那么解决循环依赖的出口是什么呢？是Spring的3级缓存机制与allowEarlyReference：
...
1. 实例化SB-A之前，先看【一级缓存】有没有SB-A，因为【一级缓存】存放的是成熟态的SB。如果存在，就没必要走实例化以及之后的操作了。**如果不存在，也不会去找二三级缓存，因为getSingleton()的allowEarlyReference是false！！！**假设现在不存在，就会走下面的流程：
2. 实例化SB-A，此时还是一个【纯净态】的SB，然后将SB对应的lambda表达式放到【三级缓存】。
3. 回顾知识点17，SB在实例化后，就会进行依赖注入，此时发现SB-A有个属性注入了SB-B，就会去创建SB-B。
4. 和SB-A一样，Spring会尝试实例化SB-B，并且在实例化之前先去【一级缓存】看有没有现成的，假设现在没有，就会继续走下面的流程：
5. 实例化SB-B，此时还是一个【纯净态】的SB，然后将SB对应的lambda表达式放到【三级缓存】。
6.  SB-B实例化后，进行依赖注入，发现SB-B有个属性注入了SB-A，就会去创建SB-A。
7. 注意！！！此时尝试实例化SB-A，getSingleton()的allowEarlyReference是true了！！！不会只检查【一级缓存】，而是【二三级缓存】都会去看。这也是为什么我说出口包含了allowEarlyReference。
8. 此时SB-A只存在【三级缓存】里，一级缓存没有，二级缓存也没有，到了三级发现有了，于是就执行SB-A的lambda表达式。如果SB-A实现了动态代理就创建动态代理并返回，如果没有实现，则直接返回这个普通的SB-A。**然后将SB-A放到【二级缓存】中**。
9. 接着继续走SB-B的生命周期流程，如果此时SB-B和SB-C还有循环依赖，也是走同样的逻辑，只不过此时SB-B充当了SB-A的角色罢了。最终SB-B会被加到【一级缓存】里
10. 最终回到SB-A属性赋值那一步，继续进行SB-A的初始化。因**为初始化阶段涉及动态代理的创建，所以在这一步会找【二级缓存】，看看SB-A会不会在解决循环依赖期间提前创建动态代理了，如果有就不重复创建了。**
11. 最终将SB-A放入【一级缓存】，并且清除掉在【二三级缓存】的存在。
